...
# a short README, and — for executed attacks — a GROUND_TRUTH answer key, or — for
# reconstructions of real incidents — a RECONSTRUCTION narrative plus a generated CLAIMS
# warranting layer. The gallery is a tour of what
...
readme() {  # dir title lookfor reproduce
  ...
  # Emulation-mode samples have ground truth (something was executed). Reconstruction-mode
  # samples do not — the referent is someone else's incident, so they ship a warranted claim
  # set instead, and the file names say which is which.
  [ -f "samples/$dir/GROUND_TRUTH.md" ] && gt=$'\n- Answer key: [`GROUND_TRUTH.md`](GROUND_TRUTH.md) — the labelled ATT&CK ground truth'
  [ -f "samples/$dir/RECONSTRUCTION.md" ] && gt="$gt"$'\n- Narrative: [`RECONSTRUCTION.md`](RECONSTRUCTION.md) — vantage, kill chain, honesty markers, residual tells'
  [ -f "samples/$dir/CLAIMS.md" ] && gt="$gt"$'\n- Warranting: [`CLAIMS.md`](CLAIMS.md) — generated by `packetforge warrant`: which source claim licenses each flow, and what is deliberately not modelled'
...
- Two **honesty markers kept on purpose** so a bare pcap still reveals itself as synthetic: every external attacker IP sits in RFC 5737 documentation ranges (\`192.0.2/24\`, \`203.0.113/24\`), and the stolen AWS keys are AWS's published EXAMPLE values. Attack TLS is all 1.3 (certs encrypted). See [\`RECONSTRUCTION.md\`](RECONSTRUCTION.md) for the kill chain, ATT&CK mapping, and the honest list of residual tells.
- **It fails Gate 4, on purpose.** [\`CLAIMS.md\`](CLAIMS.md) is generated by \`packetforge warrant\`, which checks every rendered flow against the claim set built from those two disclosure posts. It fails on every class the gate has: flows rendered with no source licensing them (the stage-2 pull, the SSH hop, the exfil lookup), source claims neither rendered nor declared unmodelled, violated magnitude floors, and a flow asserting something a source explicitly left open. The live count is in [\`CLAIMS.md\`](CLAIMS.md); it is not repeated here, because a number copied into prose drifts from the artifact — which is the CIC-IDS2017 failure this whole layer exists to avoid. Every one corresponds to an error the post-mortem later confirmed — from the sources this capture already had.
