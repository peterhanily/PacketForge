...
def write_ground_truth(intrusion: Intrusion, md_path, json_path=None) -> None:
    ...
    if json_path:
        ...
        payload = {"synthetic": True,
                   "disclaimer": ("SYNTHETIC — generated by PacketForge. No packet in this "
                                  "capture was taken from a real network; every address, domain "
                                  "and credential is fabricated. External addresses are RFC 5737 "
                                  "documentation ranges and domains are RFC 2606 reserved names. "
                                  "Do not block, hunt, or report these indicators."),
                   "title": intrusion.title, "iocs": intrusion.iocs,
...
# substituting a fake one would destroy the lesson. Every entry needs a reason, and a test
# asserts nothing else allocated ever reaches an answer key.
WELL_KNOWN_SERVICE_ADDRS = {
    "169.254.169.254": "cloud instance metadata service — link-local, the same on every provider",
    "168.63.129.16":   "Azure platform DNS / WireServer — the azure-vnet environment's own resolver",
    "1.1.1.1":         "Cloudflare public DNS — the DoH sample's point is a host resolving via a "
                       "well-known PUBLIC resolver instead of its internal one; a fabricated "
                       "resolver address would not be that behaviour",
    "8.8.8.8":         "Google public DNS — same reason",
    "100.100.100.100": "mesh-VPN MagicDNS anycast address",
}

# Real domains a sample may name, each because the service's identity IS the lesson. Everything
# else invented must sit on an RFC 2606 reserved TLD: names on registrable TLDs have been
# shipped before, unregistered, in an attacker role, where anyone could have claimed them.
WELL_KNOWN_SERVICE_DOMAINS = {
    "cloudflare-dns.com": "public DoH resolver — the encrypted-DNS sample's finding is a host "
                          "resolving through a well-known public resolver instead of its own",
    "blob.core.windows.net": "Azure Blob storage — the cloud-exfil sample's finding is "
                             "exfiltration through a trusted cloud endpoint; the subdomain is "
                             "invented, the service is the point",
}

# Hosts that samples 18/19 name because the incident's own published disclosures name them.
# They are marked `observed` in those samples' claim sets and are the referent of the
# reconstruction; substituting fakes would make the account less honest, not more.
SOURCED_INCIDENT_HOSTS = {
    "huggingface.co", "openai.com", "github.com", "api.github.com", "pypi.org",
    "files.pythonhosted.org", "sts.amazonaws.com", "amazonaws.com",
    "controlplane.tailscale.com", "pkgs.tailscale.com", "tailscale.com",
    "mongodb.net", "hf.space",
}


def _public_ip(rng) -> str:
    return f"{rng.choice(_DOC_NETS)}.{rng.randint(1, 254)}"
...
def build_ipv6_c2(env: Environment, start_time: float, rng, *, intensity: float = 1.0,
    ...
    c2_ip = "2001:db8:c2::66"        # RFC 3849 documentation prefix
    dc = env.dns_server
    sp = _sports()
    flows = []
